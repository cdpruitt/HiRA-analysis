...
//*************************************************************
bool hira::unpackCsi(unsigned long *&point,int runno)
{
    NE = 0;
    CsIM = 0;
    ...
    for (int iadc = 0;iadc<2;iadc++)
    {
        ...
        for (int i=0;i<ADC.number;i++)
        {
            ...
            int ienergy = ADC.events[i].ADCvalue;
    ...
    for (int itdc = 0;itdc<1;itdc++)
    {
        ...
        for (int i=0;i<TDC->Ndata;i++)
        {
            ...
            {
                timeRFScintillator = itime;
                Histo->timeRFScintillator->Fill(itime/10.);
            }
            else if (id == 66)
            {
                timeA1900 = itime;
                Histo->timeA1900->Fill(itime/10.);
                if (haveBlocker)Histo->Blocker_ETOF->Fill(BlockerESum,itime);
            }
            else if (id == 67)
            {
                timeRFCyclotron = itime;
                Histo->timeRFCyclotron->Fill(itime/10.);
            }
